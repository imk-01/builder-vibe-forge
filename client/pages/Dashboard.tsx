...
const calculateActivityStatus = (
  kegiatan: Kegiatan,
): KegiatanWithDynamicStatus["dynamicStatus"] => {
  ...
  const checkTahapanWarning = (
    ...
    if (tanggalSelesai && isPast(parseISO(tanggalSelesai))) {
      const dokumenTahapan = kegiatan.dokumen.filter(
        (d) => d.tipe === tipeDokumen && d.isWajib,
      );
      if (
        dokumenTahapan.length > 0 &&
        !dokumenTahapan.every((d) => d.status === "Approved")
      ) {
  ...
  checkTahapanWarning(
    kegiatan.tanggalSelesaiPersiapan,
    "persiapan",
    "Persiapan",
  );
  checkTahapanWarning(
    kegiatan.tanggalSelesaiPengumpulanData,
    "pengumpulan-data",
    "Pengumpulan Data",
  );
  checkTahapanWarning(
    kegiatan.tanggalSelesaiPengolahanAnalisis,
    "pengolahan-analisis",
    "Pengolahan & Analisis",
  );
  checkTahapanWarning(
    kegiatan.tanggalSelesaiDiseminasiEvaluasi,
    "diseminasi-evaluasi",
    "Diseminasi & Evaluasi",
  );
  ...
  if (
    kegiatan.tanggalSelesaiDiseminasiEvaluasi &&
    isPast(parseISO(kegiatan.tanggalSelesaiDiseminasiEvaluasi))
  ) {
    status = "Selesai";
  } else if (
    kegiatan.tanggalMulaiDiseminasiEvaluasi &&
    now >= parseISO(kegiatan.tanggalMulaiDiseminasiEvaluasi)
  ) {
    status = "Diseminasi & Evaluasi";
  } else if (
    kegiatan.tanggalMulaiPengolahanAnalisis &&
    now >= parseISO(kegiatan.tanggalMulaiPengolahanAnalisis)
  ) {
    status = "Pengolahan & Analisis";
  } else if (
    kegiatan.tanggalMulaiPengumpulanData &&
    now >= parseISO(kegiatan.tanggalMulaiPengumpulanData)
  ) {
...
export default function Dashboard() {
  ...
  const [selectedActivity, setSelectedActivity] =
    useState<KegiatanWithDynamicStatus | null>(null);
  const [updateModalActivity, setUpdateModalActivity] =
    useState<KegiatanWithDynamicStatus | null>(null);
  const [activityToDelete, setActivityToDelete] = useState<Kegiatan | null>(
    null,
  );
  const [showProgressSuccessModal, setShowProgressSuccessModal] =
    useState(false);
  ...
  const filteredActivities = useMemo(() => {
    return processedActivities.filter((activity) => {
      ...
      const matchesSearch = activity.namaKegiatan
        .toLowerCase()
  ...
  const stats = useMemo(() => {
    const statusCounts = processedActivities.reduce(
      (acc, act) => {
    ...
    return {
      ...
      jumlahWarning: processedActivities.filter(
        (a) => a.dynamicStatus.warnings.length > 0,
      ).length,
    };
  }, [processedActivities]);
  ...
  const handleUpdatePPL = (
    pplId: number,
    field: keyof PPLWithProgress,
    value: string,
  ) => {
        ...
        if (p.id === pplId) {
          ...
          if (newValue > bebanKerja) {
            setAlertModal({
              ...
              message: `Nilai tidak boleh melebihi total beban kerja (${bebanKerja}).`,
            });
            return p;
          }
              ...
              case "progressSubmit":
                if (diff > 0 && p.progressOpen - diff < 0) {
                  setAlertModal({
                    isOpen: true,
                    title: "Validasi Gagal",
                    message: "Submit tidak bisa lebih besar dari Open!",
                  });
                  return p;
                }
              ...
              case "progressDiperiksa":
                if (diff > 0 && p.progressSubmit - diff < 0) {
                  setAlertModal({
                    isOpen: true,
                    title: "Validasi Gagal",
                    message: "Diperiksa tidak bisa lebih besar dari Submit!",
                  });
                  return p;
                }
              ...
              case "progressApproved":
                if (diff > 0 && p.progressDiperiksa - diff < 0) {
                  setAlertModal({
                    isOpen: true,
                    title: "Validasi Gagal",
                    message: "Approved tidak bisa lebih besar dari Diperiksa!",
                  });
                  return p;
                }
  ...
  const renderPPLProgress = (pplList: PPLWithProgress[], search: string) => (
    <div className="space-y-4">
      {pplList
        .filter((p) =>
          (p.namaPPL || "").toLowerCase().includes(search.toLowerCase()),
        )
        .map((ppl) => (
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Total Beban Kerja: {ppl.bebanKerja} {ppl.satuanBebanKerja}
                  </p>
                  <p className="text-sm text-gray-600">
                    Honor: Rp{" "}
                    {parseInt(ppl.besaranHonor).toLocaleString("id-ID")}
                  </p>
                </div>
              ...
              <div className="grid grid-cols-4 gap-4">
                <div className="text-center">
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    {ppl.progressOpen || 0} {ppl.satuanBebanKerja}
                  </div>
                </div>
                <div className="text-center">
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    {ppl.progressSubmit || 0} {ppl.satuanBebanKerja}
                  </div>
                </div>
                <div className="text-center">
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    {ppl.progressDiperiksa || 0} {ppl.satuanBebanKerja}
                  </div>
                </div>
                <div className="text-center">
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    {ppl.progressApproved || 0} {ppl.satuanBebanKerja}
                  </div>
                </div>
              </div>
              ...
              <div className="text-xs text-gray-500 text-center">
                ...
                {(ppl.progressOpen ?? 0) +
                  ...
                  (ppl.progressDiperiksa ?? 0) +
                  (ppl.progressApproved ?? 0)}{" "}
                / {ppl.bebanKerja} {ppl.satuanBebanKerja}
              </div>
            </div>
          </Card>
        ))}
                ...
                {["submit", "diperiksa", "approved"].map((field) => (
                  <div key={field} className="text-center">
                    ...
                    <Input
                      ...
                      value={
                        (ppl as any)[
                          `progress${field.charAt(0).toUpperCase() + field.slice(1)}`
                        ]
                      }
                      onChange={(e) =>
                        handleUpdatePPL(
                          ppl.id!,
                          `progress${field.charAt(0).toUpperCase() + field.slice(1)}` as keyof PPLWithProgress,
                          e.target.value,
                        )
                      }
                      className="mt-1 text-center"
      ...
      <div className="space-y-8">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Pantau progress dan kelola semua kegiatan
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7 gap-6">
          <Card className="border-l-4 border-l-bps-blue-500">
            <CardContent className="p-6">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-sm font-medium">Total Kegiatan</p>
                  <p className="text-2xl font-bold">{stats.totalKegiatan}</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-2xl font-bold">{stats.persiapan}</p>
                </div>
          ...
          <Card className="border-l-4 border-l-yellow-500">
            <CardContent className="p-6">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-sm font-medium">Pengumpulan Data</p>
                  <p className="text-2xl font-bold">{stats.pengumpulanData}</p>
                </div>
          ...
          <Card className="border-l-4 border-l-green-500">
            <CardContent className="p-6">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-sm font-medium">Pengolahan</p>
                  <p className="text-2xl font-bold">{stats.pengolahan}</p>
                </div>
          ...
          <Card className="border-l-4 border-l-indigo-500">
            <CardContent className="p-6">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-sm font-medium">Diseminasi</p>
                  <p className="text-2xl font-bold">{stats.diseminasi}</p>
                </div>
          ...
          <Card className="border-l-4 border-l-red-500">
            <CardContent className="p-6">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-sm font-medium">Jumlah Warning</p>
                  <p className="text-2xl font-bold">{stats.jumlahWarning}</p>
                </div>
          ...
          <div className="sm:w-64">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="Pengumpulan Data">
                  Pengumpulan Data
                </SelectItem>
                <SelectItem value="Pengolahan & Analisis">
                  Pengolahan & Analisis
                </SelectItem>
                <SelectItem value="Diseminasi & Evaluasi">
                  Diseminasi & Evaluasi
                </SelectItem>
              ...
              const getStageDates = () => {
                    ...
                    return (
                      <div>
                        ...
                        <p className="font-medium">
                          {formatDate(
                            activity.tanggalSelesaiDiseminasiEvaluasi,
                          )}
                        </p>
                      </div>
                    );
                ...
                return (
                  <>
                    ...
                    <div>
                      <p className="text-gray-500">Selesai {stageLabel}</p>
                  ...
                  <CardContent className="space-y-4 flex-grow flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-sm font-bold text-bps-blue-600">
                          {activity.progressKeseluruhan || 0}%
                        </span>
                      </div>
                      <Progress
                        value={activity.progressKeseluruhan || 0}
                        className="h-2"
                      />
                      ...
                      <div className="text-xs text-gray-500 flex items-center gap-1 mt-2">
                        <span>Update:</span>
                        <span className="font-medium text-bps-blue-600">
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-4 border-t mt-4">
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          handleOpenUpdateModal(activity);
                          setPplSearchUpdate("");
                        }}
                      >
                      ...
                      <Button variant="outline" size="sm" asChild>
                        <Link to={`/view-documents/${activity.id}`}>
        ...
        >
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                Detail Kegiatan: {selectedActivity?.namaKegiatan}
              </DialogTitle>
            </DialogHeader>
            {selectedActivity && (
              <div className="space-y-6 p-4">
                <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-3 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {selectedActivity.namaKetua}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-gray-500">Status:</span>
                        <Badge
                          className={cn(selectedActivity.dynamicStatus.color)}
                        >
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-bps-blue-600">
                          {getRelativeTime(selectedActivity.lastUpdated)}
                        </span>
                      </div>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="space-y-3 text-sm">
                      <div className="flex justify-between">
                        <span className="text-gray-500">Persiapan:</span>
                        <span className="font-medium">
                          {selectedActivity.tanggalMulaiPersiapan
                            ? `${format(new Date(selectedActivity.tanggalMulaiPersiapan), "dd MMM yyyy", { locale: localeID })} - ${format(new Date(selectedActivity.tanggalSelesaiPersiapan!), "dd MMM yyyy", { locale: localeID })}`
                            : "-"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500">Pengumpulan Data:</span>
                        <span className="font-medium">
                          {selectedActivity.tanggalMulaiPengumpulanData
                            ? `${format(new Date(selectedActivity.tanggalMulaiPengumpulanData), "dd MMM yyyy", { locale: localeID })} - ${format(new Date(selectedActivity.tanggalSelesaiPengumpulanData!), "dd MMM yyyy", { locale: localeID })}`
                            : "-"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {selectedActivity.tanggalMulaiPengolahanAnalisis
                            ? `${format(new Date(selectedActivity.tanggalMulaiPengolahanAnalisis), "dd MMM yyyy", { locale: localeID })} - ${format(new Date(selectedActivity.tanggalSelesaiPengolahanAnalisis!), "dd MMM yyyy", { locale: localeID })}`
                            : "-"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {selectedActivity.tanggalMulaiDiseminasiEvaluasi
                            ? `${format(new Date(selectedActivity.tanggalMulaiDiseminasiEvaluasi), "dd MMM yyyy", { locale: localeID })} - ${format(new Date(selectedActivity.tanggalSelesaiDiseminasiEvaluasi!), "dd MMM yyyy", { locale: localeID })}`
                            : "-"}
                        </span>
                      </div>
                    </div>
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-gray-600 leading-relaxed whitespace-pre-wrap break-words">
                    {selectedActivity.deskripsiKegiatan}
                  </p>
                </div>
                <div>
                  ...
                  <Tabs defaultValue="persiapan">
                    <TabsList className="grid w-full grid-cols-4">
                      ...
                      <TabsTrigger value="pengumpulan-data">
                        Pengumpulan Data
                      </TabsTrigger>
                      <TabsTrigger value="pengolahan-analisis">
                        Pengolahan & Analisis
                      </TabsTrigger>
                      <TabsTrigger value="diseminasi-evaluasi">
                        Diseminasi & Evaluasi
                      </TabsTrigger>
                    </TabsList>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-blue-700 text-sm mt-1">
                        Gunakan tombol <strong>Edit</strong> untuk mengakses dan
                        mengupload dokumen semua fase. Tombol{" "}
                        <strong>View Docs</strong> untuk melihat semua dokumen
                        yang telah diunggah.
                      </p>
                    </div>
                  </div>
        ...
        <Dialog
          open={!!updateModalActivity}
          onOpenChange={(isOpen) => {
            if (!isOpen) setUpdateModalActivity(null);
          }}
        >
          <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
            <DialogHeader>
              <DialogTitle>
                Update Progress: {updateModalActivity?.namaKegiatan}
              </DialogTitle>
            </DialogHeader>
            {updateModalActivity && (
                ...
                >
                  ...
                  <div className="relative mt-4 flex-shrink-0">
                    ...
                    <Input
                      ...
                      className="pl-10 mb-4 h-8 text-sm"
                    />
                  </div>
                  <div className="overflow-y-auto flex-grow pr-2">
                    <TabsContent value="persiapan">
                      {renderPPLUpdate(
                        localPplProgress.filter((p) => p.tahap === "persiapan"),
                        pplSearchUpdate,
                      )}
                    </TabsContent>
                    <TabsContent value="pengumpulan-data">
                      {renderPPLUpdate(
                        localPplProgress.filter(
                          (p) => p.tahap === "pengumpulan-data",
                        ),
                        pplSearchUpdate,
                      )}
        ...
        <Dialog
          open={!!warningModalContent}
          onOpenChange={() => setWarningModalContent(null)}
        >
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                Peringatan untuk: {warningModalContent?.title}
              </DialogTitle>
              <DialogDescription>
                Berikut adalah daftar peringatan yang terdeteksi untuk kegiatan
                ini.
              </DialogDescription>
            </DialogHeader>
